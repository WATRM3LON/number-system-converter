...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const OPERATOR_DEFS = {
    '+': { precedence: 1, associativity: 'L', fn: (a, b) => a + b, symbol: '+' },
    '-': { precedence: 1, associativity: 'L', fn: (a, b) => a - b, symbol: '−' },
    '*': { precedence: 2, associativity: 'L', fn: (a, b) => a * b, symbol: '×' },
    '/': {
      precedence: 2,
      associativity: 'L',
      fn: (a, b) => {
        if (Math.abs(b) < 1e-12) {
          throw new Error(`Arithmetic Error: Division by zero encountered (${a} ÷ 0). Division by zero is mathematically undefined.`);
        }
        return a / b;
      },
      symbol: '÷'
  ...
  function tokenizeExpression(expr) {
    if (!expr || expr.trim() === '') {
      throw new Error('Expression is empty. Please enter an arithmetic expression.');
    }
    ...
    while (i < len) {
      ...
      if (char in OPERATOR_DEFS) {
        ...
        const isUnary = (char === '-') && (!prevToken || prevToken.value === '(' || prevToken.type === 'operator');
      ...
      if (/[0-9]/.test(char) || char === '.') {
        let numStr = '';
        while (i < len && (/[0-9]/.test(normalized[i]) || normalized[i] === '.')) {
          numStr += normalized[i];
          i++;
        }
        if ((numStr.match(/\./g) || []).length > 1) {
          throw new Error(`Syntax Error: Invalid number literal '${numStr}' with multiple decimal points.`);
        }
        tokens.push({ type: 'number', value: parseFloat(numStr) });
        continue;
      }

      throw new Error(`Syntax Error: Unrecognized character '${char}' in expression.`);
    }
    ...
    for (let t = 0; t < tokens.length; t++) {
      ...
      if (cur.type === 'operator' && next && next.type === 'operator') {
        throw new Error(`Syntax Error: Consecutive operators '${cur.value} ${next.value}' are invalid.`);
      }
    ...
    if (last && last.type === 'operator') {
      throw new Error(`Syntax Error: Expression cannot end with operator '${last.value}'.`);
    }

    const first = tokens[0];
    if (first && first.type === 'operator' && first.value !== '-') {
      throw new Error(`Syntax Error: Expression cannot start with operator '${first.value}'.`);
    }
  ...
  function infixToPostfix(tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === 'variable' || token.type === 'number') {
        ...
        while (operatorStack.length > 0) {
          ...
          if (
            (op1Def.associativity === 'L' && op1Def.precedence <= op2Def.precedence) ||
            (op1Def.associativity === 'R' && op1Def.precedence < op2Def.precedence)
          ) {
        ...
        if (!foundOpenParen) {
          throw new Error("Syntax Error: Mismatched parentheses. Found unexpected closing ')' without a matching '('.");
        }
      }
    }
    ...
    while (operatorStack.length > 0) {
      ...
      if (top.value === '(' || top.value === ')') {
        throw new Error("Syntax Error: Mismatched parentheses. Unclosed '(' found in expression.");
      }
      outputQueue.push(top);
    }
  ...
  function evaluatePostfixRPN(postfixTokens, variableMap) {
    ...
    for (let i = 0; i < postfixTokens.length; i++) {
      ...
      if (token.type === 'variable') {
        ...
        if (!(varName in variableMap)) {
          ...
          throw new Error(`Undefined Variable: '${varName}' is not defined. Available inputs: [${definedVars}].`);
        }
        ...
        if (evalStack.length < 2) {
          throw new Error(`Syntax Error: Insufficient operands for operator '${token.value}'.`);
        }
    ...
    if (evalStack.length !== 1) {
      throw new Error("Syntax Error: Incomplete expression evaluation. Verify formula operands and operators.");
    }
  ...
  function createInputRow(initialValue = '', initialBase = 10) {
    ...
    btnRemove.addEventListener('click', () => {
      if (document.querySelectorAll('.input-row').length > 3) {
        ...
        showToast('System requires at least 3 inputs as specified in Activity instructions.', 'warning');
      }
    });
  ...
  function refreshKeypadVariableButtons() {
    ...
    const rows = inputsContainer.querySelectorAll('.input-row');
    keypadVarsContainer.innerHTML = '<span class="keypad-label">Variables:</span>';
  ...
  function insertTokenAtCursor(token) {
    ...
    const startPos = customExprInput.selectionStart || customExprInput.value.length;
    ...
    customExprInput.value = val.substring(0, startPos) + insertText + val.substring(endPos);
    customExprInput.focus();
    customExprInput.selectionStart = customExprInput.selectionEnd = startPos + insertText.length;
    calculateAndRender();
  }
  ...
  function calculateAndRender() {
    ...
    if (!allInputsValid) {
      displayError("Input characters do not match selected number system. Check inputs marked in red.");
      return;
    }

    const exprStr = customExprInput ? customExprInput.value.trim() : '(A + B - C) * D';
    ...
    try {
      ...
      const formattedExpressionStr = formatExpressionWithSubscripts(exprStr, variableMap);
      document.getElementById('expression-display').textContent = formattedExpressionStr;
      ...
      calcSteps.push(`<strong>STEP 1: Normalize Input Variables to Common Base-10 (Decimal):</strong>`);
      inputsData.forEach(d => {
        calcSteps.push(`  • Variable <strong>${d.variable}</strong> = ${d.rawStr || '0'}${BASE_SUBSCRIPTS[d.base]} → <strong>${d.decimalVal}</strong>₁₀`);
      });

      calcSteps.push(`<br><strong>STEP 2: Infix Expression Parsing & Precedence (Shunting-Yard RPN):</strong>`);
      calcSteps.push(`  • Infix Expression: <code>${exprStr}</code>`);
      const rpnDisplay = postfixTokens.map(t => t.value).join(' ');
      calcSteps.push(`  • Postfix (Reverse Polish Notation): <code>${rpnDisplay}</code>`);
      calcSteps.push(`  • Precedence Order: Parentheses <code>()</code> evaluated first, then Multiplication <code>×</code> & Division <code>÷</code>, followed by Addition <code>+</code> & Subtraction <code>−</code> (Left-to-Right Associativity).`);

      calcSteps.push(`<br><strong>STEP 3: Step-by-Step Sub-Expression Evaluation:</strong>`);
      evalResult.stepLogs.forEach(step => calcSteps.push(step));
      calcSteps.push(`  • Final Decimal Evaluation Result = <strong>${finalDec}</strong>₁₀`);

      calcSteps.push(`<br><strong>STEP 4: Convert Evaluated Result (${finalDec}₁₀) to All Number Systems:</strong>`);
      ...
      document.getElementById('arithmetic-decimal-badge').innerText = `Evaluation Error`;
      document.getElementById('steps-breakdown').innerHTML = `<span style="color:#ef4444; font-weight:700;">Evaluation halted due to:</span> ${err.message}`;
    }
  }
  ...
  if (btnBackspace) {
    btnBackspace.addEventListener('click', () => {
      ...
      if (startPos > 0 && startPos === endPos) {
        customExprInput.value = val.substring(0, startPos - 1) + val.substring(endPos);
        customExprInput.selectionStart = customExprInput.selectionEnd = startPos - 1;
      } else if (startPos !== endPos) {
        customExprInput.value = val.substring(0, startPos) + val.substring(endPos);
        customExprInput.selectionStart = customExprInput.selectionEnd = startPos;
      }
      customExprInput.focus();
      calculateAndRender();
    });
